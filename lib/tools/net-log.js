...
let _RequestCache = Class({
    ...
    initData: function(aWindow, URI) {
        let window_id = this.getWindowID(aWindow);
        if (this._data[window_id] && this._data[window_id].push) {
            this._data[window_id].push({"uri": URI});
        }
    },
    ...
    pushData: function(aWindow, aData) {
        let window_id = this.getWindowID(aWindow);
        let found = false;
        if (this._data[window_id]) {
            for (var idx in this._data[window_id]) {
                if(this._data[window_id][idx]["uri"] == aData["uri"]) {
                    this._data[window_id][idx] = aData;
                    found = true;
                    break;
                }
            }
    ...
    getRequests: function(aWindow) {
        let requests = this.getEntry(aWindow).filter(function(item){
            return item["status"] !== undefined;
        });

        if (requests.length == 0) {
            requests[0] = {
                date: aWindow.document.lastModified,
                modified: aWindow.document.lastModified,
                expires: null,
                content_type: aWindow.document.contentType,
                charset: aWindow.document.characterSet,
                size: aWindow.XMLSerializer().serializeToString(aWindow.document).length,
                headers: {},
...
exports.TabRequestsLogger = function() {
    ...
    observer.add('http-on-examine-merged-response', HttpRequestObserver);
...
TracingListener.prototype = {
    ...
    onStartRequest: function(aRequest, aContext) {
        try {
            ...
            if (this.DOMWindow !== null && this.DOMWindow !== undefined && aRequest.URI.spec) {
                RequestCache.initData(this.DOMWindow, aRequest.URI.spec);
            }
        } catch(e) {
            debug.exception(e);
        }
    ...
    onStopRequest: function(aRequest, aContext, aStatusCode) {
        ...
        if (this.DOMWindow !== null && this.DOMWindow !== undefined && aRequest.URI) {
            try {
...
let getDOMWindowForRequest = function(aRequest) {
    ...
    let window = getWindowForRequest(aRequest);
    let browser;
...
let getWindowForRequest = function(aRequest) {
...
let getRequestLoadContext = function(aRequest) {
...
let getImageInfo = function(aURI, aContentType, aData) {
    ...
    */
    ...
    try {
        ...
        debug.warn("Failed to load image information - " + aURI);
        return [0,0, false];
    }
};
