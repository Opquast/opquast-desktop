...
let RequestCache = exports.RequestCache = {
    //
    // A simple cache of requests
    //
    _data: {},
    reset: function(aWindow) {
        this._data[getWindowID(aWindow)] = [];
    },
    drop: function(aWindow) {
        let window_id = getWindowID(aWindow);
        if (this._data[window_id]) {
            delete(this._data[window_id]);
        }
    },
    append: function(aWindow, aData) {
        let window_id = getWindowID(aWindow);
    ...
    get: function(aWindow) {
        return this._data[getWindowID(aWindow)]
    }
};
...
exports.TabRequestsLogger = function() {
    ...
    tabBrowser.TabTracker({
        onTrack: function(aTab) {
            RequestCache.reset(aTab.linkedBrowser.contentWindow);
        },
        onUntrack: function(aTab) {
            RequestCache.drop(aTab.linkedBrowser.contentWindow);
        }
    });
};
...
let TabProgressListener = {
    ...
    onStateChange: function(aWebProgress, aRequest, aStateFlags, aStatus) {
        if ((aStateFlags & STATE_START) && (aStateFlags & STATE_IS_WINDOW)) {
            let window_id = getWindowID(aWebProgress.DOMWindow);
            //RequestCache[window_id] = [];
            RequestCache.reset(aWebProgress.DOMWindow);
        }
    }
};
...
TracingListener.prototype =
{
    ...
    onStopRequest: function(aRequest, aContext, aStatusCode)
    {
        ...
        let info = getRequestInfo(aRequest, this.receivedData);
        delete(this.receivedData);
        RequestCache.append(this.DOMWindow, info);
    }
};
...
let getWindowID = function(aWindow) {
...
let getRequestInfo = function(aRequest, aData) {
    let out = {
    ...
    if (out.content_type !== null && out.content_type.search(/^image\//) !== -1) {
        ...
        if (out.size > 0) {
            [out.image_info.width, out.image_info.height, out.image_info.animated] = getImageInfo(out.uri, out.content_type, aData);
        }
    }
    return out;
};
...
let getImageInfo = function(aURI, aContentType, aData) {
    ...
    */
    ...
    try {
        ...
        console.debug("Failed to load image information - " + aURI);
        return [0,0, false];
    }
};
