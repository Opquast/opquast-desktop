...
exports.exportEARL = function(aFrame, aWindow, aTests) {
    ...
    var cookies = cookieManager.getCookiesFromHost("opquast.test"); // @todo remplacer le lien en dur (static-args)
    var isLogged = false, aContentWindow = aFrame.contentWindow;
    
    req.open("GET", "http://reporting.opquast.test:8000/api/projects/", false); // @todo replace le lien dur
    ...
    if(!isLogged) {
        ...
        tabs.open("http://reporting.opquast.test:8000/"); // @todo remplacer le lien en dur (static-args)
        ...
        let loadProjects = function(evt) {
            try {
                ...
                win._sendResults = function(id, url) {
                    cookies = cookieManager.getCookiesFromHost("opquast.test"); // @todo remplacer le lien en dur (static-args)
                    
                    req.open("GET", "http://reporting.opquast.test:8000/api/inspector/" + id + "/sample/", false);
