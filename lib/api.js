...
const {saveToCSV} = require("./tools/export/csv");
...
// Execute tests
exportedAPI.executeTests = function(browser, testList) {
    try {
        let har = RequestCache.getEntry(browser.contentWindow).data;
        return tester.launchTests(browser, har, testList);
    } catch(e) {
        console.exception(e);
    }
};
...
exportedAPI.saveToCSV = saveToCSV;
