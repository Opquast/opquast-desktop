...
let getSelectedTabWindow = exports.getSelectedTabWindow = function(aWindow) {
    ...
    onLocationChange: function(aProgress, aRequest, aURI) {
        try {
            ...
            if(PanelActivator.hasEntry(location)){
                Debug("Opquast Desktop", "Reopen panel on " + location);
                panel.show();
            } else{
                Debug("Opquast Desktop", "Not reopen panel on " + location);
            }
    ...
    aPanel.window.gBrowser.tabContainer.addEventListener("TabSelect", function(event) {
        try {
            ...
            if(PanelActivator.hasEntry(getSelectedTabWindow(win).location)){
                Debug("Opquast Desktop", "Reopen panel on " + getSelectedTabWindow(win).location);
                panel.show();
            } else{
                Debug("Opquast Desktop", "Not reopen panel on " + getSelectedTabWindow(win).location);
            }
       } catch(e) { console.exception(e); }
    }, false);
};
...
exports.panelOptions = {
    ...
    onOpen: function() {
        ...
        Debug("Opquast Desktop", "Open panel on " + win.location);
    },
    onClose: function() {
        let win = getSelectedTabWindow(this.window);
        PanelActivator.dropEntry(win.location);

        Debug("Opquast Desktop", "Close panel on " + win.location);
    },
    onShow: function(aCache) {
        opquastPanel.erasePanel.call(this);
        opquastPanel.showPanel.call(this, getSelectedTabWindow(this.window), false);
    },
    onHide: function() {
        /*opquastPanel.erasePanel.call(this);
...
exports.menuItem = function() {
    ...
    var pageSourceItem = contextMenu.Item({
        ...
        onMessage: function() {
            try {
                let win = wm.getMostRecentWindow("navigator:browser");
                win._split_panels[panel_id].open();
