...
const chromeMap = require("tools/chrome_map");
...
exports.main = function(aOptions, aCallback) {
    ...
    // API configuration
    let api_pref = "extensions." + self.id + ".api_url";
    if (aOptions.staticArgs.api) {
        pref_service.set(api_pref, aOptions.staticArgs.api);
    } else {
        pref_service.set(api_pref, pref_service.get(api_pref, 'https://reporting.opquast.com/api/'));
    }
    ...
    // chrome map
    chromeMap.set(self.name, self.data.url('content/'), 'content');
    ...
    if (self.loadReason == "install") {
        ...
        // l10n
        let lang = pref_service.get("general.useragent.locale").toLowerCase();
        if (lang.split("-")[0] == "fr") {
