...
let showPanel = exports.showPanel = function(aContentWindow, aForce) {
    ...
    let loadResultsUI = function(evt) {
        try {
            frame.removeEventListener("DOMContentLoaded", loadResultsUI, true);
            let win = frame.contentWindow.wrappedJSObject;
            let e = win.document.getElementById("loader");
            e.appendChild(win.document.createTextNode(load_msg));
            ui();
        } catch(e) { console.exception(e); }
    }.bind(this);

    // Now, display page in frame
    if (aForce || cache) {
        load_msg = cache && _("oqs.loading_cache") || _("oqs.loading_tests");
        frame.addEventListener("DOMContentLoaded", loadResultsUI, true);
...
function Prefs(aWin) {
    try {
        if (prefs.showRefs) {
            this.controls.show_refs.setAttribute("checked", true);
            aWin.aoColumns[2] = null;
        } else {
            this.controls.show_refs.removeAttribute("checked");
            aWin.aoColumns[2] = {bVisible: false};
        }

        if (prefs.showTimes) {
            this.controls.show_times.setAttribute("checked", true);
            aWin.aoColumns[4] = null;
        } else {
            this.controls.show_times.removeAttribute("checked");
            aWin.aoColumns[4] = {bVisible: false};
        }

        if (prefs.showDetails) {
            this.controls.show_details.setAttribute("checked", true);
            aWin.aoColumns[5] = null;
        } else {
            this.controls.show_details.removeAttribute("checked");
            aWin.aoColumns[5] = {bVisible: false};
        }
