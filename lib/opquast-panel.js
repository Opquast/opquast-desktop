...
exports.showPanel = function(aContentWindow, aCache) {
    ...
    function frameLoad(event) {
    	if(event.target.location.search == "?run") {
    		...
    		if (PanelCache.getEntry(aContentWindow.location)) {
	    		console.log("DELETE");
				PanelCache.dropEntry(aContentWindow.location);
				aCache = null;
				_this._Launch = Launch.bind(_this, aContentWindow, _this.doc.getElementById(_this.options.id + "-frame"), null);
				_this.doc.getElementById(_this.options.id + "-launchbutton").removeEventListener("command", _this._Launch, aCache, true);
				_this.doc.getElementById(_this.options.id + "-launchbutton").addEventListener("command", _this._Launch, null, true);
				event.target.location.href = self.data.url("ui/loading.html?run");
				return false;
			}
    	} else if(event.target.location.href.split("/").pop() == "main.html" && PanelCache.getEntry(aContentWindow.location)) {
			_this.doc.getElementById(_this.options.id + "-frame").setAttribute("src", self.data.url("ui/loading.html?run"));
    	} else if(event.target.location.href.split("/").pop() == "results.html"){
...
let MainUI = function(aContentWindow, aFrame) {
	try{
	    ...
	    aFrame.addEventListener("DOMContentLoaded", function(){
			...
			Debug("Opquast Desktop", "Analyse terminée sur " + aContentWindow.location);
	    }, true);
...
let CacheUI = function(aContentWindow, aFrame, aCache) {
    ...
    aFrame.addEventListener("DOMContentLoaded", function(){
    	...
    	Debug("Opquast Desktop", "Résultats depuis le cache pour " + aContentWindow.location);
    }, true);
...
let Launch = function(aContentWindow, aFrame, aCache) {
	try{
		...
		if(_this.doc.getElementById(_this.options.id + "-launchbutton").getAttribute("label") == "Relancer" && aFrame.getAttribute("src") == self.data.url("ui/loading.html?rerun")) {
			return false;
		}
