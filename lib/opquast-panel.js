...
exports.createPanel = function() {
    ...
    product_link.addEventListener("command", function() {
        tabs.open("http://reporting.opquast.com/");
    });
...
exports.showPanel = function(aContentWindow) {
    let {_X} = utils(this.doc);
    ...
    let ui = MainUI.bind(this, aContentWindow, frame);

    let frameLoaded = function(evt) {
        frame.removeEventListener("DOMContentLoaded", frameLoaded, true);
        ui();
    };

    frame.addEventListener("DOMContentLoaded", frameLoaded, true);
...
let MainUI = function(aContentWindow, aFrame) {
    ...
    window._contentWindow = aContentWindow;
    window._requests = RequestCache.getEntry(aContentWindow);
};
