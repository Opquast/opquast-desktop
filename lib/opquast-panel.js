...
exports.createPanel = function() {
    ...
    // Menu
    let context_menu_item = _X("menuitem", {
        "id": this.options.id + "-analyze",
        "label": "Analyser avec Opquast"
    });
    this.doc.getElementById("contentAreaContextMenu").appendChild(context_menu_item);
...
exports.showPanel = function(aContentWindow, aCache) {
    ...
    _this.doc.getElementById(_this.options.id + "-frame").setAttribute("src", self.data.url("ui/main.html"));
    ...
    function frameLoad(event) {
    	if(event.target.location.search == "?run") {
    		_this.doc.getElementById(_this.options.id + "-launchbutton").doCommand();
    	} else if (event.target.location.search == "?rerun") {
    		if (PanelCache.entryExists(aContentWindow.location)) {
	    		console.log("DELETE");
				PanelCache.dropEntry(aContentWindow.location);
				aCache = null;
				_this.doc.getElementById(_this.options.id + "-launchbutton").doCommand();
			}
    	}
    	
    	if(event.target.location.href.split("/").pop() == "main.html" && PanelCache.entryExists(aContentWindow.location)) {
			_this.doc.getElementById(_this.options.id + "-frame").setAttribute("src", self.data.url("ui/loading.html?run"));
    	} 
    	
    	if(event.target.location.href.split("/").pop() == "results.html"){
    		...
    		_this.doc.getElementById(_this.options.id + "-launchbutton").label = "Relancer";
    	} else {
    		...
    		_this.doc.getElementById(_this.options.id + "-launchbutton").label = "Lancer";   		
    	}
    	
    	//_this.doc.getElementById(_this.options.id + "-frame").removeEventListener("DOMContentLoaded", frameLoad, true);
    }
    
    _this.doc.getElementById(_this.options.id + "-frame").addEventListener("DOMContentLoaded", frameLoad, true);
   	
    let ui;

    // Cache
    if (aCache) {
        ui = CacheUI.bind(this, aContentWindow, _this.doc.getElementById(_this.options.id + "-frame"), aCache);
        ...
        let loadCache = function(evt) {
            ...
            try {
				...
				//let inspector = new InspectorUI(frame.contentWindow);
				//aFrame.contentWindow.__ffinspector = inspector;
            } catch(e) { console.exception(e); }
        }

        _this.doc.getElementById(_this.options.id + "-frame").addEventListener("DOMContentLoaded", loadCache, true);
        _this.doc.getElementById(_this.options.id + "-frame").setAttribute("src", self.data.url("ui/loading.html"))
    }
    
    // Launch
    let _Launch = Launch.bind(this, aContentWindow, this.doc.getElementById(this.options.id + "-frame"), aCache);
    this.doc.getElementById(_this.options.id + "-launchbutton").addEventListener("command", _Launch, aCache, true);
...
let MainUI = function(aContentWindow, aFrame) {
	try{
	    ...
	    aFrame.addEventListener("DOMContentLoaded", function(){
			PanelCache.setEntry(aContentWindow.location, {
		        "tests": tests,
		        "checklists": window.checklists
		    });
	    	window = aFrame.contentWindow.wrappedJSObject;
	    	window.showResults(tests);
	    	Prefs(aContentWindow, aFrame);
			notifications.notify({
				title: "Opquast Desktop",
				text: "Analyse terminée sur " + aContentWindow.location
			});
	    }, true);
...
let CacheUI = function(aContentWindow, aFrame, aCache) {
    ...
    aFrame.addEventListener("DOMContentLoaded", function(){
    	window = aFrame.contentWindow.wrappedJSObject;
    	window.showResults(aCache.tests);
    	Prefs(aContentWindow, aFrame);
    	notifications.notify({
			title: "Opquast Desktop",
			text: "Résultats depuis le cache pour " + aContentWindow.location
		});
    }, true);
...
let Launch = function(aContentWindow, aFrame, aCache) {
	try{
		...
		if(_this.doc.getElementById(_this.options.id + "-launchbutton").getAttribute("label") == "Relancer") {
			aFrame.setAttribute("src", self.data.url("ui/loading.html?rerun"));
			aCache = null;
		}
		...
		if (aCache === undefined || aCache === null) {
	        ...
	        var ui = CacheUI.bind(_this, aFrame, aCache);
