...
exports.showPanel = function(aContentWindow, aCache) {
    ...
    if (aCache) {
        ...
        let loadCache = function(evt) {
            ...
            try {
				...
				//let inspector = new InspectorUI(aContentWindow);
				//aFrame.contentWindow.wrappedJSObject.__ffinspector = inspector;
            } catch(e) { console.exception(e); }
        }

        _this.doc.getElementById(_this.options.id + "-frame").addEventListener("DOMContentLoaded", loadCache, true);
        _this.doc.getElementById(_this.options.id + "-frame").setAttribute("src", self.data.url("ui/cache.html"))
    }
    ...
    this.doc.getElementById(this.options.id + "-exportcsv").addEventListener("command", function() {
        ...
        var data = "Liste;Référence;Libellé du test;Résultat;Commentaires;Détails;\n" + PanelCache.getEntry(aContentWindow.location).tests.oaa_results.filter(function(element, index, array){
