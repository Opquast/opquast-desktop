...
exports.createPanel = function() {
	...
	this.controls.launch.addEventListener("command", (function(){
		let {getSelectedTabWindow} = require("main-panel.js");
		erasePanel.call(this);
        showPanel.call(this, getSelectedTabWindow(this.window), true);
	}).bind(this));
   ...
   // Help
    ...
    this.controls.help.addEventListener("command", function() {
...
let showPanel = exports.showPanel = function(aContentWindow, aForce) {
    ...
    // Export
    this.controls.exp_csv.addEventListener("command", function() {
        var results = {
            "c": _("oqs.pass"),
            "nc": _("oqs.fail"),
            "i": _("oqs.cannot_tell"),
    ...
    // Search
    this.controls.search.addEventListener("input", function(event){
        let window = frame.contentWindow.wrappedJSObject;
        window.oTable.fnFilter(event.target.value);
    });

    // Show
    this.controls.show_refs.addEventListener("command", function(event) {
        let window = frame.contentWindow.wrappedJSObject;
    ...
    this.controls.show_times.addEventListener("command", function(event) {
        let window = frame.contentWindow.wrappedJSObject;
    ...
    this.controls.show_details.addEventListener("command", function(event) {
        let window = frame.contentWindow.wrappedJSObject;
...
function Prefs(_this, win) {
    try {
        ...
        if (get("extensions.opquast-desktop.showRefs") == undefined) {
            set("extensions.opquast-desktop.showRefs", _this.controls.show_refs.hasAttribute("checked"));
        } else {
        ...
        if (get("extensions.opquast-desktop.showTimes") == undefined) {
            set("extensions.opquast-desktop.showTimes", _this.controls.show_times.hasAttribute("checked"));
        } else {
        ...
        if (get("extensions.opquast-desktop.showDetails") == undefined) {
            set("extensions.opquast-desktop.showDetails", _this.controls.show_details.hasAttribute("checked"));
        } else {
