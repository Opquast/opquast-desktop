...
let SplitDock = exports.SplitDock = function SplitDock(aOptions) {
    ...
    let delegate = {
        onTrack: function(aWindow) {
            ...
            if (!aWindow._split_docks) { aWindow._split_docks = {}; }
            aWindow._split_docks[options.id] = options.dockClass(aWindow, {
                "id": options.id,
        ...
        onUntrack: function(aWindow) {
            ...
            let dock = aWindow._split_docks[options.id];
            if (dock) {
                dock.destroy();
            }
        }
...
let BaseDock = exports.BaseDock = Class({
    ...
    init: function init() {
        ...
        let before = this.chromeDoc.getElementById(this.insert_before_id);
        before.parentNode.insertBefore(this.deck, before);
    },
