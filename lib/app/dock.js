...
let _DockCache = Class({
    ...
    addDock: function(aChromeWindow, aOptions) {
        let docks = this.getEntry(aChromeWindow) || {};
        docks[aOptions.id] = aOptions.dockClass(aChromeWindow, {
            "id": aOptions.id,
            "stylesheet": aOptions.stylesheet
        });
        this.setEntry(aChromeWindow, docks);
        console.debug('dock added to DockCache');
    },

    removeDock: function(aChromeWindow, aID) {
        let docks = this.getEntry(aChromeWindow);
    ...
    getDock: function(aChromeWindow, aID) {
        let docks = this.getEntry(aChromeWindow);
        if (docks) {
            return docks[aID];
        }
...
let SplitDock = exports.SplitDock = function SplitDock(aOptions) {
    ...
    let delegate = {
        onTrack: function(aChromeWindow) {
            // Create dock on every new window
            if (aChromeWindow.location != "chrome://browser/content/browser.xul") {
                return;
            }

            DockCache.addDock(aChromeWindow, options);
            console.debug('SplitDock created');
        },
        onUntrack: function(aChromeWindow) {
            // Remove dock when extension is disabled or removed
            if (aChromeWindow.location != "chrome://browser/content/browser.xul") {
                return;
            }

            DockCache.removeDock(aChromeWindow, options.id);
            DockCache.dropEntry(aChromeWindow);
