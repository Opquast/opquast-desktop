...
const checklists = {
   ...
   "716-rgaa": {
        name: "RGAA V3",
        html: '<acronym title="Référentiel Général d\'Accessibilité pour les Administrations − Version 3">RGAA V3</acronym>',
        langs: ["fr-fr"],
        category: _("oqs.check_a11y"),
 ...
 * @param object   har    har data
 * @param array testList  list of test ids
 */
let launchTests = function(browser, har, testList) {
    ...
    har.entries.forEach(function(entry) {
        if (plainText !== "") return;
        if (entry._url === browser.documentURI.spec) {
    ...
    .then(function(resultData) {
        let {pageInfo, resources, results } = resultData;
