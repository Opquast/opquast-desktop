...
let launchTests = exports.launchTests = function(aWindow, aRequests) {
    ...
    let {env} = require('api-utils/environment');

    let checklists = {};
    for each(let filename in checklistsFiles["fr"]) {
        if(prefs["use" + filename + "Fr"]) {
           var data = JSON.parse(self.data.load(TESTS_ROOT + "fr/" + filename + ".json"));
            for (var attrname in data) {
                checklists[attrname] = data[attrname];
            }
        }
    }
    for each(let filename in checklistsFiles["en"]) {
        if(prefs["use" + filename + "En"]) {
            var data = JSON.parse(self.data.load(TESTS_ROOT + "en/" + filename + ".json"));
            for (var attrname in data) {
                checklists[attrname] = data[attrname];
            }
        }
    }
    sandbox["checklists"] = checklists;
