...
let OpquastPanel = exports.OpquastPanel = Class({
    ...
    initToolbar: function initToolbar() {
        ...
        this.controls.show_times.addEventListener("command", this.tbShow.bind(this, "showTimes", 4), false);
    ...
    disableControls: function disableControls() {
        ...
        if (!prefs.debug) {
            this.controls.show.setAttribute("hidden", true);
        }
    },
    ...
    setScroll: function setScroll() {
        //this.iframe.scroll(this.scroll);
    ...
    resultsUI: function resultsUI(aCache) {
        ...
        if (aCache) {
            ...
            console.debug("Results from cache for " + domWindow.location);
        } else {
            ...
            console.debug("Analyze done on " + domWindow.location);
        }
    ...
    tbSearch: function tbSearch(evt) {
        let win = this.iframe.contentWindow.wrappedJSObject;
        win.jQuery("#test_result").dataTable().fnFilter(evt.target.value);
    },

    tbShow: function tbShow(aPrefName, aColNum, evt) {
        let win = this.iframe.contentWindow.wrappedJSObject;

        let oTable = win.jQuery("#test_result").dataTable();
