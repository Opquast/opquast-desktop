...
let ExportReportingPanel = exports.ExportReportingPanel = Class({
    ...
    showUI: function showUI() {
        this.loaderWorker(_('oqs.loading_projects')).port.on('ready', function() {
    ...
    showProjects: function showProjects(projects) {
        let worker = this.frameWorker(null, self.data.url("ui/projects.js"));

        worker.port.on("readyProjects", function() {
            worker.port.emit("showProjects", projects);
        });

        worker.port.on("sendResults", function(projectId, projectName) {
            if(!promptService.confirm(this.tabWindow, "Confirmation", _("oqs.confirmDataSending", unescape(projectName)))) {
                return false;
            }

            this.loaderWorker(_("oqs.sending_results")).port.on("ready", function() {
