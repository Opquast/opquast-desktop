...
let ExportReportingPanel = exports.ExportReportingPanel = Class({
    ...
    sendResults: function sendResults(projectID, sampleID, sampleName) {
        this._worker.port.emit("showMessage", _("oqs.sending_results"));

        this.injectRequest(projectID, sampleID)
        .on("complete", this.onInjectDone.bind(this, projectID, sampleID, sampleName))
        .post();
    },
        ...
        try {
            if (response.status == 200 || response.status == 304) {
                ...
                if (!samples || samples.length == 0) {
                    ...
                    this._worker.port.emit("showSamples", samples, projectID, projectName);
                }
            } else {
                this.showError(_("oqs.alert_unexpected_error"));
            }
    ...
    onInjectDone: function onInjectDone(projectID, sampleID, sampleName, response) {
        try {
            if (response.status == 201) {
                this._worker.port.emit("showMessage",
                    _("oqs.alert_data_send", unescape(sampleName)),
                    'sent',
                    "OK"
                );
                return;
            } else if (response.status == 403) {
                this.showError(_("oqs.alert_new_page_forbiden"));
            } else if (response.status == 409) {
                this.showError(_("oqs.alert_task_running"))
            } else {
                this.showError(_("oqs.alert_unexpected_error"));
            }
        } catch(e) {
    ...
    samplesRequest: function samplesRequest(projectId) {
        return Request({
            url: this.baseURI + "projects/" + projectId + "/samples",
            contentType: "application/json",
            headers: {
                "Accept": "application/json",
                "Authorization": "Basic " + this.auth
    ...
    injectRequest: function injectRequest(projectID, sampleID) {
        let data = this.tests;
