...
let {Symbiont} = require('api-utils/content');
...
const {on, once} = require("tools/events");
...
const BasePanel = exports.BasePanel = Class({
    ...
    initUI: function initUI() {
        ...
        once(this.iframe, "DOMContentLoaded", function() {
            emit(this, "ready");
        }.bind(this))
    ...
    frameWorker: function frameWorker(options, additionalScripts) {
        options = mix({
            ...
            "contentScriptFile": [
                ...
                self.data.url("lib/jquery-ui-1.8.22.custom.min.js"),
            ...
            "contentScriptOptions": {
                ...
                "templates": {
                    'tplLoader': self.data.load("ui/templates/loader.tpl"),
                    'tplLanding': self.data.load("ui/templates/landing.tpl"),
        ...
        if (additionalScripts) {
            options.contentScriptFile.push(additionalScripts);
        }

        return Symbiont(options);
    },

    loaderWorker: function frameLoader(aMessage) {
        let worker = this.frameWorker();
        worker.port.on("ready", function() {
            worker.port.emit("showMessage", aMessage);
        });
