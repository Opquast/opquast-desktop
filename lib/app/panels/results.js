...
const ResultPanel = exports.ResultPanel = Class({
    ...
    initialize: function initialize(aParent, aOptions) {
        ...
        internals(this).worker = null;
        internals(this).tests = null;
    },
    ...
    initToolbar: function initToolbar() {
        ...
        on(this.controls.close, "command", _dock.close.bind(_dock), false);
        ...
        on(this.controls.launch, "command", this.tbLaunch.bind(this), false);
        ...
        on(this.controls.exp_csv, "command", this.tbExportCSV.bind(this), false);
        ...
        on(this.controls.exp_reporting, "command", this.tbExportReporting.bind(this), false);
        ...
        on(this.controls.help, "command", this.tbHelp.bind(this))
        ...
        on(this.controls.search, "command", this.tbSearch.bind(this));
        ...
        on(this.controls.show_refs, "command", this.tbShow.bind(this, "showRefs", "hRef"), false);
        ...
        on(this.controls.show_themas, "command", this.tbShow.bind(this, "showThemas", "hThema"), false);
        ...
        on(this.controls.show_times, "command", this.tbShow.bind(this, "showTimes", "hDuration"), false);
    ...
    showUI: function showUI(aForce) {
        ...
        if (aForce || cache) {
            ...
            this.loaderWorker(load_msg).port.on("ready", this.resultUI.bind(this, cache));
        } else {
    ...
    resultUI: function resultUI(aCache) {
        ...
        let worker = this.frameWorker(null, self.data.url("ui/results.js"));
        internals(this).worker = worker;
        internals(this).tests = tests;
        ...
        // Show results!
        worker.port.on("readyResults", function() {
            worker.port.emit("showResults", tests, tableOptions);
        }.bind(this));

        // Some events from results
    ...
    tbExportCSV: function tbExportCSV(evt) {
        if (internals(this).tests) {
            exportCSV(this.parent.dock.getCurrentTabWindow(), internals(this).tests.oaa_results);
        }
    },

    tbExportReporting: function tbExportReporting(evt) {
        if (internals(this).tests) {
            ExportReportingPanel(this.parent, internals(this).tests);
            this.deck.selectedIndex = this.deck.childNodes.length - 1;
        }
    },

    tbSearch: function tbSearch(evt) {
        if (internals(this).worker) {
            internals(this).worker.port.emit("resultSearch", evt.target.value);
        }
    },
    ...
    tbShow: function tbShow(aPrefName, aColName, evt) {
        if (internals(this).worker) {
            ...
            internals(this).worker.port.emit("changeColVisibility", aColName, prefs[aPrefName]);
        }
    },
