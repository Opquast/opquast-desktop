...
const ResultPanel = exports.ResultPanel = Class({
    ...
    initToolbar: function initToolbar() {
        ...
        this.controls.keep_values = _X("checkbox", {
            "label" : _("oqs.keep_values"),
            "checked": true
        });
        ...
        $(toolbar, "vbox>hbox")[0].appendChild(this.controls.keep_values);
    ...
    showUI: function showUI(aForce) {
        ...
        .then(function(entry) {
            ...
            worker.port.on("setUserData", function(test_id, result) {
                ResultStore.setUserData(domWindow.location.href, test_id, result).then(function() {
    ...
    enableControls: function enableControls(worker) {
        ...
        this.controls.keep_values.removeAttribute("hidden");
    ...
    disableControls: function disableControls() {
        ...
        this.controls.keep_values.setAttribute("hidden", true);
        this.showInfo("");
    ...
    toggleKeepValues: function toggleKeepValues() {
        let hasUserData = this._tests.oaa_results.filter(function(v) !v.auto).length > 0;
        if (hasUserData) {
            this.controls.keep_values.removeAttribute("hidden");
            this.controls.keep_values.checked = true;
        }
        else {
            this.controls.keep_values.setAttribute("hidden", true);
        }
    ...
    resultUI: function resultUI(aCache) {
        ...
        let removeUserData = !this.controls.keep_values.checked;
...
let Store = Class({
    ...
    setUserData: function(aURL, aTestID, aResult) {
        return this.dropUserData(aURL, [aTestID])
        .then(function() {
            return this.db.query(
                "INSERT INTO " + this.tableUserData +
                " (url, test_id, result) VALUES (:url, :test_id, :result)",
                {"url": aURL, "test_id": aTestID, "result": aResult}
            );
        }.bind(this))
        .then(null, function(e) {
            if (prefs.debug) console.exception(e);
    ...
    getUserData: function(aURL) {
        return this.db.query("SELECT * FROM " + this.tableUserData + " WHERE url = :url", {
            "url": aURL
        })
        .then(function(data) {
            return data || [];
