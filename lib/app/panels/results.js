...
const ResultPanel = exports.ResultPanel = Class({
    ...
    tbExportCSV: function tbExportCSV(evt) {
        try {
            if (this._tests) {
                exportCSV(this.tabURL, this._tests.oaa_results);
            }
        }
        catch(e) {
            console.exception(e);
    ...
    inspectElement: function inspectElement(aPath) {
        try {
            inspectNode(this.browserWindow, aPath);
        } catch(e) {
            if (prefs.debug) {
...
const legacyInspectNode = function(browserWindow, aPath) {
    let tabWindow = browserWindow.gBrowser
        .getBrowserForTab(browserWindow.gBrowser.selectedTab)
        .contentWindow;
    let node = tabWindow.document.querySelector(aPath);
    if (node === null) {
        return;
    }
    let TargetFactory;
    ...
    browserWindow.gDevTools.showToolbox(target, "inspector").then(function(toolbox) {
        toolbox.getCurrentPanel().selection.setNode(node, "gcli");
    });
};


const inspectNode = function(browserWindow, path) {
    let TargetFactory;
    let gDevTools;
    try {
        let {devtools} = Cu.import("resource://devtools/shared/Loader.jsm", {});
        TargetFactory = devtools.TargetFactory;
        gDevTools = Cu.import("resource://devtools/client/framework/gDevTools.jsm", {}).gDevTools;
    }
    catch(e) {
        // Firefox <~38
        legacyInspectNode(browserWindow, path);
        return;
    }
    let target = TargetFactory.forTab(browserWindow.gBrowser.selectedTab);
    let walker;
    ...
    gDevTools.showToolbox(target, "inspector").then(toolbox => {
        currentPanel = toolbox.getCurrentPanel();
        if ("walker" in currentPanel) {
            // Firefox >~52
            walker = currentPanel.walker;
            return walker.getRootNode();
        }
        else {
            // Firefox =~45
            currentPanel.selection.setNode(path, "gcli");
            return null;
        }
    }).then(rootNode => {
        if (!rootNode) {
        ...
        return walker.querySelector(rootNode, path);
    }).then(nodeFront => {
        if (!nodeFront) {
            return null;
        }
        currentPanel.selection.setNodeFront(nodeFront, "gcli");
    });
};
