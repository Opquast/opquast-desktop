        ...
        if (aCache) {
            ...
            setTimeout(function(d) {
                ...
                launchTests(domWindow, har).then(function(result) {
                    ...
                    ResultStore.setCombinedResults(domWindow.location.href, result)
                    .then(function() {
...
let Store = Class({
    ...
    setCombinedResults: function(aURL, aData) {
        // Removed test ids in user_data for url
        let ids = aData.tests.oaa_results.map(function(v) {
            return v.id;
        });

        //return this.setResults(aURL, aData);
        return this.dropUserData(aURL, ids)
        .then(this.setResults.bind(this, aURL, aData));
    },
